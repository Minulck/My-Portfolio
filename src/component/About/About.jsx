...
const About = () => {
  ...
  useEffect(() => {
    // Skip animation if mobile
    if (isMobile) {
      return;
    }

    // Desktop: run terminal animation
    let timeouts = [];
    let totalDelay = 0;
  ...
  }, [codeLines, handleTriggers, isMobile]);
  ...
  const renderCodeLine = useCallback((line, index) => {
    ...
    const highlightSyntax = (text) => {
      ...
      if (text.includes("age:")) {
        return (
          <>
            <span className="text-blue-300 font-medium"> age</span>
            <span className="text-white">: </span>
        ...
        return (
          <>
            ...
            <span className="text-green-300">
              `Undergraduate Student University of Moratuwa`
            </span>
          </>
        );
      }
    ...
    return (
      <div
        key={index}
        className="flex items-center group hover:bg-gray-800/30 px-2 py-0.5 rounded transition-colors"
      >
        <span className="text-gray-500 w-8 text-right mr-4 select-none text-xs font-mono">
          {(index + 1).toString().padStart(2, "0")}
        </span>
        <div className="flex-1">{highlightSyntax(code)}</div>
      </div>
    );
  }, []);
  ...
  return (
    <section
      id="about"
      className="px-[5vw] md:px-[7vw] lg:px-[10vw] font-sans mt-16 md:mt-24 lg:mt-22  mb-5 md:mb-24 lg:mb-30 "
    >
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 min-h-[70vh]">
        {/* Left Section - Enhanced Terminal (Hidden on Mobile) */}
        ...
        {/* Right Section - Enhanced Details (Full width on mobile, half width on desktop) */}
          ...
          >
            <div className="relative">
              <p className="text-gray-300 text-base lg:text-lg leading-relaxed max-w-lg mx-auto hover:text-gray-200 transition-colors duration-300">
                Passionate developer who loves creating innovative solutions and
                learning new technologies every day. I transform ideas into
                beautiful, functional digital experiences with clean, efficient
                code.
              </p>
